...
<!-- ultrathink: 주간 데이터를 깊이 분석하여 패턴과 트렌드를 도출합니다 -->
...
2. **journal-writer** — Agent 도구로 실행. `1-analysis.md`를 읽고 + 아래 마크다운 구조/설정을 포함하여 저널 초안 작성. 일별 데이터를 단순 나열이 아닌 **패턴과 트렌드**로 종합할 것을 지시. Agent 프롬프트 텍스트 첫 줄에 반드시 `ultrathink`를 포함하세요. 결과를 `$TMPDIR/memoir-journal-pipeline/2-draft.md`에 저장
